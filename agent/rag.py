...
mais similares por score >= threshold (0.75). O
...
# Single-tenant: uma unica base de conhecimento, sem filtro por agente.
_SEARCH_SQL = """
...
async def retrieve(query: str) -> list[RagHit]:
    ...
    log.info(
        "RAG retrieve",
        extra={"fields": {"hits": len(hits), "query_len": len(query)}},
    )
    return hits
